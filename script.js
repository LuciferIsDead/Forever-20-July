...
function showVault(){
  ...
  back.disabled=false;
  hint.textContent='A few things I never want you to forget.';addVaultSecret();
}
...
/* =====================================
   VERSION 1.6 INTERACTIVE LAYER
   Built on the stable Version 1.5 story.
===================================== */

function addRevealInteraction(label, message, icon='✨'){
  const visual = document.getElementById('chapterVisual');
  if(!visual) return;

  const old = visual.querySelector('.v16-interaction');
  if(old) old.remove();

  const wrap = document.createElement('div');
  wrap.className = 'v16-interaction';

  const button = document.createElement('button');
  button.type = 'button';
  button.className = 'v16-reveal-button';
  button.textContent = label;

  const reveal = document.createElement('div');
  reveal.className = 'v16-reveal-message';
  reveal.hidden = true;
  reveal.innerHTML = `<span class="v16-reveal-icon">${icon}</span><span>${message}</span>`;

  button.addEventListener('click',()=>{
    reveal.hidden = !reveal.hidden;
    button.textContent = reveal.hidden ? label : 'Hide this memory';
  });

  wrap.append(button,reveal);
  visual.appendChild(wrap);
}

function enhanceChapterInteraction(){
  const messages = [
    ['Tap to continue the conversation', 'A simple conversation became the beginning of something neither of us expected.', '💬'],
    ['Tap the little joke', 'It was supposed to be fake. Somewhere along the way, it stopped feeling completely fake.', '🎭'],
    ['Tap the heart', 'Some feelings arrive quietly. Then one day, you realize they are impossible to ignore.', '❤️'],
    ['Look closer', 'You did not see me that day. But I saw you.', '👀'],
    ['Let the rose bloom', 'A small flower. A memory I still remember clearly.', '🌹'],
    ['Touch the band', 'Not just a hair band. Our Engagement Band.', '💍'],
    ['Follow the journey', 'I travelled all that way just to be near you. Even the missed Churi delivery became part of our story.', '🚆'],
    ['Tap the window', 'Even when we could not meet properly, I still found a way to be close to you.', '🪟'],
    ['Choose a memory', 'Laser Tag. Haunted House. Laughter. And finally, a day that felt like ours.', '🎡'],
    ['Tap 4:15 AM', 'The longest call we had. And somehow, the best sleep of my life came after it.', '🌅'],
    ['Tap the stars', 'More memories. More journeys. More calls. More laughs. More us.', '♾️']
  ];

  const item = messages[current];
  if(item) addRevealInteraction(item[0], item[1], item[2]);
}

function addHomeDiscovery(){
  const bg = document.querySelector('#homeScreen .background');
  if(!bg || bg.querySelector('.v16-home-star')) return;

  const star = document.createElement('button');
  star.type = 'button';
  star.className = 'v16-home-star';
  star.setAttribute('aria-label','A hidden memory');
  star.textContent = '✦';

  const note = document.createElement('div');
  note.className = 'v16-home-note';
  note.hidden = true;
  note.textContent = 'Some memories are hidden in the little things. ❤️';

  star.addEventListener('click',()=>{
    note.hidden = !note.hidden;
    star.classList.toggle('is-found', !note.hidden);
  });

  bg.append(star,note);
}

function addIntroDiscovery(){
  const stars = document.querySelector('#introScreen .stars');
  if(!stars || stars.dataset.v16Ready) return;

  stars.dataset.v16Ready = 'true';
  stars.addEventListener('click',()=>{
    stars.classList.add('v16-star-touched');
    setTimeout(()=>stars.classList.remove('v16-star-touched'),700);
  });
}

function setupMusic(){
  const music = document.getElementById('perfectMusic');
  ...
  button.addEventListener('click', async()=>{
    if(music.paused){
      try{
        ...
        button.textContent = '🎵';
        button.title = 'Add assets/audio/perfect-edm.mp3 to enable music';
      }
    }else{
      music.pause();
      button.textContent = '🎵';
...
function addVaultSecret(){
  const holder = document.querySelector('.vault-grid');
  if(!holder || holder.querySelector('.v16-secret')) return;

  const secret = document.createElement('button');
  secret.type = 'button';
  secret.className = 'v16-secret';
  secret.innerHTML = '<span>✦</span><small>There may be one more thing...</small>';

  secret.addEventListener('click',()=>{
    secret.classList.toggle('revealed');
    secret.querySelector('small').textContent = secret.classList.contains('revealed')
      ? 'You are still my favourite part of this story. ❤️'
...
relive.addEventListener('click',()=>{home.classList.add('hidden');story.classList.remove('hidden');story.classList.remove('finale-mode');render(0)});
